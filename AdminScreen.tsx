...
import { View, Text, Button, FlatList, Alert, StyleSheet, Linking } from 'react-native';
...
const AdminScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get('http://192.168.9.103:5000/api/user');
  ...
  const fetchUserDetails = async (userId: string) => {
    try {
      const response = await axios.get(`http://192.168.9.103:5000/api/user/${userId}`);
  ...
  const handleWeeklyReport = async () => {
    try {
      const response = await axios.get('http://192.168.9.103:5000/api/scan/generate-excel-report', { responseType: 'blob' });
