...
const LoginScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const handleRegister = async () => {
    ...
    try {
      const response = await axios.post('http://192.168.9.103:5000/api/auth/register', { username, password });
  ...
  const handleLogin = async () => {
    ...
    try {
      const response = await axios.post('http://192.168.9.103:5000/api/auth/login', { username, password });
